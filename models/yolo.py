...
# from models.extra_modules.RepVGG import *
# from models.extra_modules.GSConv import *
# from models.extra_modules.attention import *
# from models.extra_modules.DBB import *
# from models.extra_modules.PConv import *
...
class Decoupled_Detect(nn.Module):
    # YOLOv5 Decoupled Detect head for detection models
    stride = None  # strides computed during build
    dynamic = False  # force grid reconstruction
    export = False  # export mode
...
class DetectionModel(BaseModel):
    ...
    #加载传入配置文件
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes ‘init’函数负责搭建整个网络结构
        super().__init__()
        if isinstance(cfg, dict):   #判断是否为字典格式
        ...
        m = self.model[-1]  # Detect()   取出模块的最后一层（detect层）
        if isinstance(m, (Detect, Decoupled_Detect, Segment)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, Segment) else self.forward(x)
            m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward    得到stride大小
            check_anchor_order(m)       #判断anchors顺序有没有写反
            m.anchors /= m.stride.view(-1, 1, 1)
            self.stride = m.stride
            self._initialize_biases()  # only run once

        # Init weights, biases  初始化网络参数
        initialize_weights(self)
        self.info()
        LOGGER.info('')

    def forward(self, x, augment=False, profile=False, visualize=False):    #‘forward’函数对输入图片进行预测   前向传播
        if augment:
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency    处理输出的bias，处理后收敛更快
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors, nc, gd, gw, act = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple'], d.get('activation')     #从yaml文件中取出各个参数
    if act:
        Conv.default_act = eval(act)  # redefine default activation, i.e. Conv.default_act = nn.SiLU()
        LOGGER.info(f"{colorstr('activation:')} {act}")  # print
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors  ‘anchors[0]’第一行anchor   'len'得到长度6  6/2表示anchor数量
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5)   模型最终输出的通道数  每一个特征层级在3个anchor上预测，乘3  5表示4+1,4为待检测目标的坐标，1为类别

    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch out(输出的通道数)
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings    判断m是否为一个类 e.g. 'models.common.Conv' 'eval()'判断字符串是否为可执行函数
        for j, a in enumerate(args):
            with contextlib.suppress(NameError):
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings

        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain    n=n*gd
        if m in {
                ...
                GSConv, VoVGSCSP, RepVGGBlock, BottleneckEMA, C3_EMA}:           #判断m属于哪种结构
            #卷积层
            c1, c2 = ch[f], args[0] #c1=3   c2为args的第0个参数
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)     #c2=c2*gw   判断是否为8的倍数，若不是，变为8的倍数

            args = [c1, c2, *args[1:]]  #’*args[1:]‘为args后三位数（卷积核尺寸、步长、padding大小）   ’c1‘为输入尺寸   ’c2‘为输出尺寸   拼接在一起
            if m in {BottleneckCSP, C3, C3TR, C3Ghost,
                     VoVGSCSP, C3_EMA}:
                args.insert(2, n)  # number of repeats  在原列表第二个位置插入n
                n = 1
        ...
        t = str(m)[8:-2].replace('__main__.', '')  # module type    字符串第8到倒数第二位
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print    打印输出信息
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist     统计那些层需要保存
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5l.yaml', help='model.yaml')       #‘cfg’配置文件
